...
parser.add_argument('--move', type=str2bool, default=False, help='Whether to randomly move the image for data augmentation')
...
parser.add_argument('--layers', type=int, default=16, help='The number of weight layers. Only supported by the Encoder-Decoder model')
parser.add_argument('--name', type=str, default='model', help="The name of the trained model")
...
network, init_fn = model_builder.build_model(model_name=args.model, frontend=args.frontend, net_input=net_input, num_classes=num_classes, crop_width=args.crop_width, crop_height=args.crop_height, is_training=True, layers=args.layers)
...
model_checkpoint_name = "checkpoints/%s/latest_model_" % args.name + args.model + "_" + args.dataset + ".ckpt"
...
for epoch in range(args.epoch_start_i, args.num_epochs):
    ...
    if not os.path.isdir("%s/%s/%04d"%("checkpoints",args.name,epoch)):
        os.makedirs("%s/%s/%04d"%("checkpoints",args.name,epoch))
    ...
    if val_indices != 0 and epoch % args.checkpoint_step == 0:
        ...
        saver.save(sess,"%s/%s/%04d/model.ckpt"%("checkpoints",args.name,epoch))


    if epoch % args.validation_step == 0:
        print("Performing validation")
        target=open("%s/%s/%04d/val_scores.csv"%("checkpoints",args.name,epoch),'w')
        ...
        for ind in val_indices:
            ...
            cv2.imwrite("%s/%s/%04d/%s_pred.png"%("checkpoints",args.name,epoch, file_name),cv2.cvtColor(np.uint8(out_vis_image), cv2.COLOR_RGB2BGR))
            cv2.imwrite("%s/%s/%04d/%s_gt.png"%("checkpoints",args.name,epoch, file_name),cv2.cvtColor(np.uint8(gt), cv2.COLOR_RGB2BGR))
