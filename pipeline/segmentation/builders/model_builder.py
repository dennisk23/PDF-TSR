...
def download_checkpoints(model_name):
    subprocess.check_output(["python3", os.path.join(os.path.dirname(os.path.realpath(__file__)), "../utils/get_pretrained_checkpoints.py"), "--model=" + model_name])



def build_model(model_name, net_input, num_classes, crop_width, crop_height, frontend="ResNet101", is_training=True, layers=16):
	...
	if model_name == "FC-DenseNet56" or model_name == "FC-DenseNet67" or model_name == "FC-DenseNet103":
	    ...
	    raise ValueError("Error: the model %d is not available. Try checking which models are available using the command python3 main.py --help")

	print("Loaded model", model_name, "with frontend", frontend)
