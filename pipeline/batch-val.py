...
def remove_from_json(json_folder, name):
    ...
    with open(os.path.join(json_folder, base_name.split("-Table")[0]+".json"), "r+") as jfile:
        tables = json.load(jfile)
        tables = list(filter(lambda table: not table['name'] == "Table" + base_name.split("-Table")[-1], tables))
        jfile.seek(0)
...
if __name__ == '__main__':
    ...
    if not opt.skip_generate_images:
        ...
        os.system('java -jar ./pipeline/pdffigures2.jar -e -q -a Table -m ' + png_folder + '/ -d ' + json_folder + '/ ' + pdf_folder + '/')

    if not opt.skip_pad_images:
    ...
    if not opt.skip_predict:
        ...
        if opt.model == 'pix2pixHD':
            subprocess.call(['python', './pipeline/pix2pixHD/predict.py', '--name', '.', '--checkpoints_dir', opt.checkpoint_dir,  '--dataroot', opt.dataroot, '--loadSize', '1024', '--fineSize', '1024', '--no_instance', '--label_nc', '0', '--results_dir', outlines_folder, '--mode', opt.mode, '--which_epoch', opt.epoch])
        elif opt.model == 'encoder-decoder-skip':
            subprocess.call(['python', './pipeline/segmentation/bulk_predict.py', '--input_folder', png_folder, '--output_folder', outlines_folder, '--checkpoint_path', opt.checkpoint_dir, '--crop_height', '1024', '--crop_width', '1024', '--model', 'Encoder-Decoder-Skip'])
        else:
            print("Unknown model")
            exit(-1)
    ...
    if not opt.skip_create_csv:
        print("Creating csv")
        json2csv2.json2csv(json_folder, results_folder)
        #json2csv2.json2csv(json_folder, results_folder, True)
